...
const config = {
	description: 'Marks a suggestion as potential with your reason.',
	aliases: [],
...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		embed.setColor(client.config.colors.LIGHTYELLOW);
		embed.setAuthor(oldEmbed.author.name, oldEmbed.author.iconURL);
		embed.setDescription(oldEmbed.description);
		embed.addFields({
			name: `Marked potential by ${message.author.tag} (${message.author.id})`,
			value: reason
		});
		embed.setFooter(
