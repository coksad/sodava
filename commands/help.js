...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		if (commandQuery) {
			let command = client.commandList.find(
				c =>
					c.name.toLowerCase() === commandQuery.toLowerCase() ||
					c.config.aliases
						.map(a => a.toLowerCase())
			...
			if (command) {
				...
				if (command.config.rolesRequired.length !== 0)
					embed.addField(
						'Roles Required',
						command.config.rolesRequired.join(', '),
						true
					);
				embed.setColor(client.config.colors.PURPLE);
				embed.setAuthor(message.author.tag, message.author.displayAvatarURL());
				embed.setFooter(`Powering Great Service`, `https://cdn.discordapp.com/attachments/798629244091367444/798629782296199219/1c933d00cb0ccddcff3292dd5f4e4227-2.png`);
		...
		for (const categoryName of Object.keys(categories)) {
			...
			let commandString = category
				.map(
					c =>
						`\`${client.config.prefix}${c.name}${
							c.config.usage ? ` ${c.config.usage}` : ''
