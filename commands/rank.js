...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		if (
			client.config.maximumRank <= rankInGroup ||
			client.config.maximumRank <= rank
		) {
			embed.setDescription(
		...
		let rankingInfo;
		try {
			rankingInfo = await roblox.setRank(
				client.config.groupId,
				id,
				Number(rank)
			);
		...
		embed.setDescription(
			`**Success!** I have ranked **${username}** from **${rankNameInGroup} (${rankInGroup})** to **${
				rankingInfo.name
			} (${rankingInfo.rank})** in the group.`
		);
